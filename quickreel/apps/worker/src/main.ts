import { createAnalysisWorker, createRenderWorker } from "@quickreel/queue";
...
console.log("QuickReel worker started — listening on analysis-queue and render-queue.");

async function shutdown() {
  console.log("Shutting down worker...");
  await Promise.all([analysisWorker.close(), renderWorker.close()]);
  process.exit(0);
}
